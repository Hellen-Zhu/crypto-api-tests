-- ====================================================================
-- Update WebSocket Negative Test Validation Rules (Full Field Validation v2)
-- ====================================================================
-- This script updates validation rules to verify ALL meaningful fields
-- EXCEPT the "id" field which is dynamic and returned by the server
--
-- Based on actual API responses collected on 2025-10-13
-- Key finding: API returns id=-9223372036854775808 for malformed messages
-- ====================================================================
...
-- Response: {"id": -9223372036854775808, "method": "", "code": 40003, "message": "No such method"}
-- Note: id is a large negative number (Long.MIN_VALUE), method is empty string
UPDATE case_data_sets
SET validations_override = '{
  "3": {
    "body": {
...
-- Response: {"id": -9223372036854775808, "method": "subscribe", "code": 40003, "message": "Channels param empty"}
UPDATE case_data_sets
SET validations_override = '{
  "3": {
    "body": {
...
-- Response: {"id": -9223372036854775808, "method": "invalid_method", "code": 40003, "message": "No such method"}
UPDATE case_data_sets
SET validations_override = '{
  "3": {
    "body": {
...
-- 2. Some responses have additional "channel" field (when channel is recognizable)
-- 3. Using placeholders like {{@invalid_channel}} for dynamic validation
-- 4. Empty method ("") for missing method field scenario
-- 5. NOT validating "id" field - it's dynamic and can be -9223372036854775808 for errors
-- 6. This validates ALL meaningful fields, making it easier to maintain
-- ====================================================================
